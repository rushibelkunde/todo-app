...
const TodoList = ({ token, categories, fetchCategories }) => {
  ...
  const fetchTodos = () => {
    ...
    axios.get(`${Url}/todos`, {
      ...
      .then(response => {
        ...
        if (category !== "all") {
          const categorizedTodos = filteredTodos.filter(todo => todo.category_id == category)
          setCurrentTodos(categorizedTodos.slice(indexOfFirstTodo, indexOfLastTodo));
        }
        else {
  ...
  return (
    <div>
      <AddTodoForm token={token} onSearch={handleSearch} setCurrentTodos={setCurrentTodos} onAdd={fetchTodos} fetchCategories={fetchCategories} categories={categories} />
